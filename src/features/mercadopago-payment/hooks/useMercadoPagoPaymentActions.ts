...
} from '@/features/mercadopago-payment/hooks/useMercadoPagoPayment.ts'
import {
  PaymentRequest,
  PayerInfo,
...
export const usePaymentActions = (options?: PaymentActionsOptions) => {
  ...
  const createQuickPayment = useCallback(
    async (
      amount: number,
      currency: string,
      phone: string,
      ...
      const payerInfo = createPayerFromUser(
        phone,
        identification,
        identificationType
      )
      return createPayment(amount, currency, payerInfo, customDescription)
    },
...
export const usePaymentTracking = (
  ...
  const paymentPollingQuery = usePaymentStatusPolling(paymentId || '', {
    ...
    stopWhen: (status) =>
      ['approved', 'rejected', 'cancelled'].includes(status.toLowerCase()),
  })

  // Usar la query de polling si está habilitado, si no el básico
  const activeQuery = options?.enablePolling
    ? paymentPollingQuery
    : paymentStatusQuery
